...
import {
  ...
  type UserResponse, type CreateUserRequest, type UpdateUserRequest,
} from '../../api/users';

const DEFAULT_CREATE: CreateUserRequest = {
  email: '', firstName: '', lastName: '', roleId: 1, password: `Welcome@${new Date().getFullYear()}!`,
};
...
export default function PlatformUsersPage() {
  ...
  const [createForm, setCreateForm] = useState<CreateUserRequest>(DEFAULT_CREATE);
  const [editForm, setEditForm] = useState<UpdateUserRequest>({ firstName: '', lastName: '', roleId: 1 });
  ...
  async function load() {
    setLoading(true);
    try { setUsers((await getUsers(true)).filter(u => u.role === 'SuperAdmin')); }
  ...
  function openEdit(u: UserResponse) {
    setEditForm({ firstName: u.firstName, lastName: u.lastName, roleId: u.roleId });
    setError(''); setModal(u);
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setModal(null);
      showToast(`Platform user created. Temp password: ${createForm.password}`);
      load();
    } catch (e: unknown) {
  ...
  async function handleReset(u: UserResponse) {
    const tempPwd = `Welcome@${new Date().getFullYear()}!`;
    if (!confirm(`Reset password for ${u.fullName} to "${tempPwd}"?`)) return;
    try {
      await resetPassword(u.userId, tempPwd);
      showToast(`Password reset. New temp password: ${tempPwd}`);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Platform Users</h1>
          <p className="text-sm text-gray-500 mt-0.5">Super admins who can access this portal.</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? <div className="p-8 text-center text-gray-400">Loading…</div> :
          users.length === 0 ? <div className="p-8 text-center text-gray-400 text-sm">No platform users found.</div> : (
                  ...
                  <tr key={u.userId} className="hover:bg-gray-50">
                    <td className="px-5 py-3">
                      ...
                      {u.mustChangePassword && (
                        <span className="ml-2 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">pwd reset</span>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-3 flex gap-3 items-center">
                      ...
                      <button onClick={() => handleReset(u)}
                        className="text-gray-500 hover:text-gray-700 text-xs font-medium">Reset Pwd</button>
      ...
      {modal === 'create' && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 space-y-4">
            <h2 className="text-lg font-semibold text-gray-900">New Platform User</h2>
            ...
            <div className="bg-gray-50 rounded-lg px-4 py-3 text-sm text-gray-600">
              Temporary password: <span className="font-mono font-medium text-gray-800">{createForm.password}</span>
              <p className="text-xs text-gray-400 mt-0.5">User must change this on first login.</p>
            </div>
            ...
            <div className="flex gap-3 justify-end pt-1">
              ...
              <button onClick={handleCreate} disabled={saving}
                className="px-5 py-2 bg-gray-800 hover:bg-gray-900 text-white text-sm font-medium rounded-lg disabled:opacity-50">
